...
def spike_amp_plot_asset(ax, waveform_mean, waveform_sem, n_spikes, cmap, invert=False, DENSITY_BINS=DENSITY_BINS, yicks=yticks, ytick_labels=yticklabels):
    """
    ...
    N = n_spikes  # number of spikes used for mean
    sigma = waveform_sem * np.sqrt(N)

    bin_centers = 0.5 * (DENSITY_BINS[1:] + DENSITY_BINS[:-1])

    data = np.array([
        np.exp(-0.5 * ((bin_centers - m) / s)**2)
        for m, s in zip(waveform_mean, sigma)
    ])

    data /= data.sum(axis=1, keepdims=True)  # normalize per timepoint
    ...
    ax.set_xticks([0,63])
    ax.set_xticklabels([0, 2], fontsize=ticklabelsize, )
    ...
    ax.set_yticks(yicks)
    ax.set_yticklabels(ytick_labels, color=c, fontsize=ticklabelsize)
...
def firingrate_plot_asset(ax, ax_raster, act_z, bins, 
    ...
    ax.set_yticklabels([round(ylims[0],1), round(ylims[1],1)], color=c, fontsize=labelsize)
    ax.set_xticks(ticklabels)
    ax.set_xticklabels(ticklabels, color=c, fontsize=labelsize)
