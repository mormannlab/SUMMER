"""
PyTorch Dataset and Lightning DataModule for SUMMER NWB data.
...
BUFFER_NUMBER_REPEATED_PARTS = {
    4: 59, 8: 36, 12: 26, 16: 21, 20: 17, 24: 14, 28: 12, 32: 11, 36: 10, 40: 9, 44: 8,
}

VALID_BUFFERS = list(BUFFER_NUMBER_REPEATED_PARTS.keys())
VALID_FOLDS = [1, 2, 3, 4, 5]
VALID_PATIENT_IDS = set(str(i) for i in range(1, 30))  # "1" through "29"

from ML_framework.nwb_loading.nwb_loading import (
    get_annotation_labels_nwb,
...
def _validate_patient_ids(patient_ids):
    """Raise ValueError if any patient ID is outside the valid range 1–29."""
    invalid = [pid for pid in patient_ids if pid not in VALID_PATIENT_IDS]
    if invalid:
        raise ValueError(
            f"Invalid patient ID(s): {invalid}. "
            f"Valid patient IDs are 1 to 29."
        )

class SUMMERDataset(Dataset):
    """
...
class SUMMERDatasetLabels(Dataset):
    """
    Dataset of binned spike counts with a single annotation label per bin (frame).
    Each sample is (spikes_at_bin, label_at_bin): input (n_units,) and target class index.
    Supports one or multiple patients; multiple patients are concatenated along the units axis.
    Labels are loaded from the first patient's NWB (same movie for all).
    """
    ...
    def __init__(
        ...
        Args:
            ...
            bin_length: Bin length in ms for spike binning. Default: 80.
            brain_regions: Optional list of brain regions to restrict units.
            label_name: Name of the annotation label to predict (e.g. "summer").
        """
        if nwb_path is not None:
            patient_id, data_dir = _patient_and_dir_from_path(nwb_path)
            patient_ids = [patient_id]
        ...
        # Load binned spikes (same as SUMMERDataset)
        list_binned = []
        n_bins_ref = None
        for pid in self.patient_ids:
            units = get_unit_ids_for_patient_nwb(
            ...
            if n_bins_ref is not None and binned.shape[1] != n_bins_ref:
                raise ValueError(
                    f"Patient {pid} has {binned.shape[1]} bins, but previous patient(s) have {n_bins_ref}. "
                    "All patients must have the same number of bins."
                )
            n_bins_ref = binned.shape[1]
            list_binned.append(binned)
        ...
        # Load labels from first patient (same movie for all)
        self.labels, self.n_classes = get_annotation_labels_nwb(
            self.patient_ids[0],
        ...
        if len(self.labels) != self.n_bins:
            raise ValueError(
                f"Label length {len(self.labels)} does not match n_bins {self.n_bins}."
            )
        print(
            f"Shape of binned spikes (patients {self.patient_ids}): {self.binned_spikes.shape}; "
            f"labels '{label_name}': {self.labels.shape}, n_classes={self.n_classes}\n"
        )
...
class SUMMERDataModule(pl.LightningDataModule):
    """Lightning DataModule for SUMMER NWB data (single or multiple patients).
    ...
    def __init__(
        ...
        """
        Args:
            buffer: If set, use buffer/fold split. Must be in [5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55] (seconds). Default: 40.
            fold: When using buffer split, which fold is validation (1–5). Must be set together with buffer. Default: 1.
        ...
        if self.patient_ids is None:
            _validate_patient_ids([self.patient_id])
        self.bin_length = bin_length
        self.brain_regions = brain_regions
...
class SUMMERDataModuleLabels(pl.LightningDataModule):
    """Lightning DataModule for SUMMER NWB data with frame-level annotation labels.
    Same split options as SUMMERDataModule (fraction or buffer/fold).
    """
    ...
    def __init__(
        ...
        self.save_hyperparameters(
            ignore=["nwb_path", "data_dir", "patient_id", "patient_ids", "brain_regions"]
        )
        self.nwb_path = nwb_path
        self.data_dir = data_dir or DEFAULT_NWB_DIR
        self.patient_id = patient_id
        ...
        if buffer is not None:
            ...
            if fold is None or fold not in VALID_FOLDS:
                raise ValueError(
                    f"fold must be in {VALID_FOLDS} when buffer is set, got {fold}"
                )
        if fold is not None and buffer is None:
            raise ValueError("fold requires buffer to be set")
    ...
    def _prepare_split_indices(self):
        ...
        logger.info(
            "Data split (labels): buffer=%s s, sequence=%s, n=%s, base=%s, buffer_bins=%s, fold_len=%s, fold=%s",
            self.buffer,
            self.sequence,
            n,
            base,
        ...
        for k in range(number_repeated_parts):
            ...
            fold2 = list(
                range(
                    i + fold_len + buffer_bins,
                    i + 2 * fold_len + buffer_bins,
                    step,
                )
            )
            buffer2 = list(
    ...
    def setup(self, stage=None):
        self.dataset = SUMMERDatasetLabels(
            nwb_path=self.nwb_path,
            data_dir=self.data_dir,
            patient_id=self.patient_id,
            patient_ids=self.patient_ids,
